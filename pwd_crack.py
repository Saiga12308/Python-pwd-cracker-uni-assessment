...
class pwd_crack:
    def __init__(self, worker_count):
        ...
        for i in range(delta.days + 1):
            ...
            self.date_list.append(day[0]+day[1]+day[2])
            self.date_list.append(day[2]+day[1]+day[0])
    ...
    def crack_password(self, pid):
        #function for loops for cracking password. done this way to be able to utilise multiprocessing

        gen = WordVarGen()

        #used for slices for threads
        len_pwd_file = len(self.lines)

        slice_a = int((pid/self.worker_count) * len_pwd_file)
        slice_b = int(((pid+1) / self.worker_count) * len_pwd_file)

        list_pwds = self.lines[slice_a:slice_b]
        ...
        for line in dict_file:
            ...
            variants = gen.word_variant_unhashed(word, self.date_list)
    ...
    def start(self):
        ...
        pool.map(self.crack_password, range(self.worker_count))